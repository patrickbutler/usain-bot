...
class LocalBackend(StorageBackend):
    ...
    def find_duplicate_activity_groups(self) -> list[list[Activity]]:
        """Same physical run stored under different Garmin IDs: same date +
        type, start times within 2 minutes (or both missing), distances
        within 5%. Sorted so the best row to keep (longest raw payload,
        i.e. richest record) is first in each group."""
        activities = self.get_activities()
        groups: list[list[Activity]] = []
        used: set[str] = set()
        by_key: dict[tuple, list[Activity]] = {}
        for a in activities:
            by_key.setdefault((a.date, a.activity_type), []).append(a)
            ...
            for i, a in enumerate(bucket):
                ...
                for b in bucket[i + 1:]:
                    ...
                    if a.start_time and b.start_time:
                        close_in_time = abs((a.start_time - b.start_time).total_seconds()) <= 120
                    else:
                        close_in_time = a.start_time is None and b.start_time is None
                    max_dist = max(a.distance_mi, b.distance_mi, 0.01)
                    similar_distance = abs(a.distance_mi - b.distance_mi) / max_dist <= 0.05
                    if close_in_time and similar_distance:
                        group.append(b)
                        used.add(b.activity_id)
                if len(group) > 1:
                    used.add(a.activity_id)
                    group.sort(key=lambda x: len(json.dumps(x.raw)), reverse=True)
                    groups.append(group)
        return groups
