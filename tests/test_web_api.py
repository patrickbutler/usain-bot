...
class TestDataPipelineEndpoints:
    ...
    def test_backfill_reports_data_quality_inline(self, client):
        body = client.post("/api/backfill").json()
        # dedupe/validation are part of ingest now, so their outcome is
        # reported by the pipeline rather than a separate endpoint
        assert "rejected_count" in body
        assert "repaired_count" in body
        assert "deduped_count" in body

    def test_sync_reports_data_quality_inline(self, client):
        body = client.post("/api/sync").json()
        assert body["rejected_count"] == 0
        assert body["deduped_count"] == 0

    def test_manual_dedupe_endpoint_is_gone(self, client):
        assert client.post("/api/dedupe").status_code == 404
