...
class TestAutomaticDedupe:
    """Deduplication is a property of ingest, not a command. These assert
    it happens on the normal write paths without anyone asking."""

    def test_clean_data_has_nothing_to_dedupe(self, storage, fixture_path):
        adapter = MockGarminAdapter(fixture_path)
        result = agent.sync_activities(storage, adapter, date(2026, 7, 27))
        assert result.deduped_count == 0

    def test_sync_removes_same_run_under_two_ids(self, storage):
        ...
        result = agent.sync_activities(storage, adapter, date(2026, 7, 21))
        assert result.deduped_count == 1
        assert len(storage.get_activities()) == 1

    def test_backfill_removes_duplicates_automatically(self, config, storage):
        adapter = MockGarminAdapter.from_activities([
            act("1", date(2026, 7, 20), 6.0, start_hour=7),
            act("2", date(2026, 7, 20), 6.0, start_hour=7),
        ])
        result = agent.backfill_history(config, storage, adapter, as_of=date(2026, 7, 27),
                                         sleep_fn=lambda s: None)
        assert result.deduped_count >= 1
        assert len(storage.get_activities()) == 1

    def test_dedupe_keeps_the_richer_record(self, storage):
        rich = replace(act("1", date(2026, 7, 20), 6.0, start_hour=7),
                       raw={"activityId": 1, "detail": "lots", "more": [1, 2, 3]})
        sparse = replace(act("2", date(2026, 7, 20), 6.0, start_hour=7), raw={})
        adapter = MockGarminAdapter.from_activities([sparse, rich])
        agent.sync_activities(storage, adapter, date(2026, 7, 21))
        remaining = storage.get_activities()
        assert len(remaining) == 1
        assert remaining[0].activity_id == "1"

    def test_split_run_parts_are_never_deduped(self, storage):
        # Two deliberate recordings of one long run 25 min apart: sessions.py
        # merges these at READ time; they must both survive in storage.
        adapter = MockGarminAdapter.from_activities([
    ...
    def test_different_days_never_deduped(self, storage):
        adapter = MockGarminAdapter.from_activities([
            act("1", date(2026, 7, 20), 6.0), act("2", date(2026, 7, 21), 6.0),
        ])
        result = agent.sync_activities(storage, adapter, date(2026, 7, 22))
        assert result.deduped_count == 0
        assert len(storage.get_activities()) == 2

    def test_no_manual_dedupe_command_exists(self):
        """Deliberately removed — if this ever comes back, ingest regressed."""
        assert not hasattr(agent, "dedupe_activities")
        from usain_bot import cli
        parser = cli.build_parser()
        subcommands = set(parser._subparsers._group_actions[0].choices)
        assert "dedupe" not in subcommands
